...
window.addEventListener('scroll', function(){
  var top = document.getElementById('mini-logo');
  var topdiv = document.getElementById('divColorChange');
  var psitionY = window.pageYOffset;
  if (psitionY > 100) {
    top.classList.remove('display-none');
    topdiv.classList.remove('firstColor');
    topdiv.classList.add('secondColor');
  } 
  else {
    top.classList.add('display-none');
    topdiv.classList.add('firstColor');
    topdiv.classList.remove('secondColor');
  }
})
...
function selectionPromo() {
  ...
  console.log(limYearProm.event.target);
